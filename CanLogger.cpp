...
int main(int argc, char* argv[])
{
  ...
  if (argc >= 2)
  {
    if (use_xml)
    {
      if (!KXmlParser::ParseFile(av1, &Root))
        return -1;
      if (argc > 2)
      {
    ...
    if (NCanUtils::GetDriverType(av1) == NCanUtils::dt_unknown &&
        !KIpSocket::IsHostAddr(av1))
    {
  ...
  printf("device: %s\ndriver type: %s\n", Can.GetDev(), GetDriverTypeStr(Can.DriverType));
