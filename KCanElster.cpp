...
KCanElster::KCanElster(bool UseMqtt)
{
  Trace = false;
  SendCanId = 0x680;
#if defined(__MQTT__)
...
{
  DriverType = NCanUtils::dt_can232;
  AsSimulation = true;
...
bool KCanElster::InitELM327(const char * com_dev, bool trace)
{
  DriverType = NCanUtils::dt_elm327;
  AsSimulation = true;
  if (!Init(com_dev, 0x680))
    return false;
...
int main(int argc, char* argv[])
{
  ...
  bool mqtt = NCanUtils::IsMqtt(argv[argc-1], mqtt_addr);
  if (mqtt)
    argc--;
  ...
  {
    argc--;
    Can.DriverType = NCanUtils::dt_cs;
  }
  ...
  if (!params_ok)
  {
    printf("use scanner\n"
           ...
           "or read or change value\n"
           "  can_scan <can dev> <sender can id> <receiver can id> [ \".\" <elster index> [ \".\" <new value> ] ] [ \"cs_com\" ] [ \"trace\" ]\n\n"
  #if defined(__WINDOWS__)
           "example: can_scan com3 680 180           (scan all elster indices from 0000 to 1fff)\n"
           "or       can_scan com3 680 180.0126      (read minutes at elster index 0126)\n"
           "or       can_scan com3 680 180.0126.0f00 (set minutes to 15)\n\n"
           "allowed sender can id: 680 to 69f, 700 to 71f, and 780 to 79f\n"
  #else
           "example: can_scan ttyUSB0 680 180        (scan all elster indices from 0000 to 1fff)\n"
           "or       can_scan can0 680 180.0126      (read minutes at elster index 0126)\n"
           "or       can_scan can0 680 180.0126.0f00 (set minutes to 15)\n\n"
           "allowed sender can id: 680 to 69f, 700 to 71f, and 780 to 79f\n"
  #endif
  #if defined(__ROTEX__)
           "                       100 to 11f, 180 to 19f, and 300 to 31f\n"
  #endif
           "\n"
           "DO NOT USE 680 TOGETHER WITH WPM3\n\n"
...
int main(int argc, char* argv[])
{
  ...
  bool mqtt = false;
#if defined(__MQTT__)
  mqtt = NCanUtils::IsMqtt(argv[argc-1], mqtt_addr);
  if (mqtt)
    argc--;
#endif
...
#if defined(__MYSQL__) && !defined(__MQTT__)
...
int main(int argc, char* argv[])
{
  ...
  CanElster.DriverType = NCanUtils::dt_cs;
  ...
  do
  {
    ...
    if (logfile.GetNextFrame(CanElster.SendFrame))
    {
      if (CanElster.SendFrame.Id != 0x680 && CanElster.SendFrame.Data[0] != 0xd2)
      {
        if (trace)
          CanElster.PrintSendFrame();
        if (CanElster.SendData(CanElster.SendFrame))
          KScanTable::SetValue(CanElster.SendFrame);
      }
    }
