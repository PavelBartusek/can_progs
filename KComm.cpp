...
// setzt die Parameter(Baudrate, RTS und DTR).
  ...
  if (!COM_INITIALISIERT() && (FComPortNr != 0 || FComDev[0]))
  {
    if (FComDev[0] && strlen(FComDev) < 8)
      strcpy(Nr, FComDev);
    else
...
// setzt die Parameter(Baudrate, RTS und DTR).
  ...
  if (!COM_INITIALISIERT() &&
      (FComPortNr != 0 || strlen(FComDev) > 0))
  {
    if (strlen(FComDev))
    {
      Nr[0] = 0;
      if (strncmp(FComDev, "/dev/", 5))
        strcpy(Nr, "/dev/");
      strcat(Nr, FComDev);
    } else
...
void KComm::SetComDev(const char * tty_dev)
{
  ...
  if (strlen(tty_dev) < High(FComDev))
    strcpy(FComDev, tty_dev);
}
