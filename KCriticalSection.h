 ...
 */
  ...
  class KRingBuffer
  {
    private:
      unsigned FrameCount;
      KCriticalSection * Semaphore;
      int RingBufferSize;
      volatile int RingHead;
      volatile int RingTail;
      FrameType * RingBuffer;

      void IncRingPtr(volatile int & RingPtr);
      void DecRingPtr(volatile int & RingPtr);
    public:
      KRingBuffer();
      virtual ~KRingBuffer();
      int  RingCount() const;
      bool RingBufferFull() const;
      bool Pop(FrameType & Frame);
      bool GetNext(FrameType & Frame);
      bool Push(const FrameType & Frame);
  };
