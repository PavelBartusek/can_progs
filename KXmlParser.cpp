...
void KXmlParser::GetTagName(int & aTagNamePos, TTagName & aTagName)
{
  NeedChar('<');
  SkipSpaces();
...
bool KXmlParser::IsActualPositionValid()
{
  if (mStatusOk)
  {
    char Ch = SkipSpaces();
...
int KXmlParser::GetIdentifier(char * Id, int MaxIdLen)
{
  int Len = 0;
...
bool KXmlParser::ParseXmlHeader()
{
...
void KXmlParser::SetError(int ErrPos)
{
  if (mStatusOk)
  {
    mStatusOk = false;
...
bool KXmlParser::StartNewBody(KXmlNode * NewNode)
{
  bool Comment;
  char Ch;
  int  Pos;
...
void KXmlParser::TestBodyEnd(TTagName & TagName)
{
...
void KXmlParser::NeedChar(char Char)
{
  if (mStatusOk)
  {
    char ch = SkipSpaces();
...
bool KXmlParser::TestChar(char Char)
{
  if (mStatusOk)
  {
    char ch = SkipSpaces();
...
bool KXmlParser::GenerateFromXml(KXmlNode * Parent)
{
  int TagNamePos;
  TTagName TagName;
...
bool KXmlParser::ParseFile_(const char * FileName, KXmlNode * Root)
{
...
bool KXmlParser::Parse(KXmlNode * Root)
{
  Initialize();
  ParseXmlHeader();
...
bool KXmlParser::ParseFile(const char * FileName, KXmlNode * Root)
{
  KXmlParser Parser;
