 ...
 */
  ...
  namespace NCanUtils
  {
    ...
    bool IsMqtt(const char * arg, const char * AddrAndPort);
    enum driver_type GetDriverType(const char * dev);
    const char * GetDriverTypeStr(driver_type type);
  };
  ...
  struct KCanELM327Frame
  {
    // T14611234 4 00010203             1 + 8 + 1 + 2*8 = 26 Byte max.
    // t550      8 aabbccddeeff0a0b
    char msg[32];

    KCanELM327Frame(const char * str = 0);
    bool SetCanFrame(KCanFrame & Frame) const;
    bool GetCanFrame(const KCanFrame & Frame);
    bool IsOk() const;
    KCanELM327Frame & operator =(const char * str);
  };
