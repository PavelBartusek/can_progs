             ...
             bool binary_protocol, void * Form)
{
  ...
  if (dev == dev_can232 || dev == dev_usb2can)
  {
    ...
    if (dev == dev_usb2can)
    {
      ...
      CanElster->DriverType = NCanUtils::dt_8dev;
    } else {
  #endif
      sprintf(port, "COM%d", sec_com_nr);
      CanElster->DriverType = NCanUtils::dt_can232;
  #if defined(__WINDOWS__)
    }
  #endif
  ...
  {
    CanElster = new KCanElster;
    CanElster->DriverType = NCanUtils::dt_can232_remote;
...
{
#if defined(__WINDOWS__)
  void * pwine_get_version = NULL;
  HINSTANCE__ * hnd = GetModuleHandle("ntdll.dll");
  if (hnd)
    pwine_get_version = GetProcAddress(hnd, "wine_get_version");
#if defined(CONSOLE) && 0
  if (pwine_get_version != NULL)
    prifnt("wine version: %s\n", pwine_get_version);
#endif
...
#if defined(__232_CLIENT__)
int main(int argc, char* argv[])
{
  printf("COM232 Server\n"
         "copyright (c) 2021 Juerg Mueller, CH-5524\n\n");
  ...
  {
    printf("usage:\n\n"
           "  com232_server <COM232 serial port> <CAN232 IP Address>\n\n");
    return 1;
  }

  KCan232 can232;
  KTcpCanClient client;
  const char * tpc_address = argv[2];
  const char * can_dev = argv[1];
...
int main(int argc, char* argv[])
{
  ...
  if (argc < 2 || argc > 3)
  {
    printf("usage:\n\n"
  #if defined(__WINDOWS__)
           "  cs_bruecke_dos <serial port to ComfortSoft> [ <CAN port> | <CAN232 IP Address] [ <scan_data table> ]\n\n"
