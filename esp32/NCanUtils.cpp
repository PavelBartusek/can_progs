...
const char * NCanUtils::GetDriverTypeStr(driver_type type)
{
  switch (type)
  {
  #if defined(__LINUX__) || !defined(__MAC__)
    case dt_can:    // SocketCAN: für PICAN CAN-Bus Board oder USB2CAN von 8devices.com
      return "socket can";
  #endif
    case dt_can232: // RS232: CAN232, CANUSB von Lawicel oder USBtin von Fischl.de
      return "USBtin";
  #if !defined(__NO_REMOTE__)
    case dt_can232_remote: // CAN232 via TCP-Client
      return "remote";
  #endif
    case dt_elm327:
      return "elm327";
    case dt_cs:     // optisches Interface: ComfortSoft-Protokoll
      return "optical adapter";
  #if defined(__WINDOWS__)
    case dt_8dev:    // Interface von 8devices.com - verwendet usb2com.dll (only for windows)
      return "usb2com";
  #endif
    default:
      return "unknown";
...
bool NCanUtils::IsMqtt(const char * arg, const char * AddrAndPort)
{
  bool mqtt = !strncmp((char *) arg, "mqtt", 4);
  AddrAndPort = NULL;
  if (mqtt)
  {
    // mqtt@192.168.0.62:1883
    arg += 4;
    if (*arg++ == '@')
      AddrAndPort = arg;
  };
...
{
  // 0x.79 ist keine ID (passive Abfrage)
  //  480 00 [7] a0 79 fa 4e 30 00 00
  //  { 0x579, 0x4e30, 0x0000},
...
int KCanFrame::GetIdToValue() const
{
  if (Len < 6)
    return -1;
...
void KCanFrame::SetCanFrame(can_frame & frame) const
{
  frame.can_id = Id;
  frame.can_dlc = Len;
  for (int i = 0; i < Len; i++)
...
KCanELM327Frame::KCanELM327Frame(const char * str)
{
  unsigned len = 0;
  if (str)
    len = (unsigned) strlen(str);
...
bool KCanELM327Frame::SetCanFrame(KCanFrame & Frame) const
{
  Frame.Init();
...
bool KCanELM327Frame::IsOk() const
{
  KCanFrame Frame;
...
bool KCanELM327Frame::GetCanFrame(const KCanFrame & Frame)
{
  sprintf(msg, "ATSH%3.3X", Frame.Id & 0x7ff);
...
KCanELM327Frame & KCanELM327Frame::operator =(const char * str)
{
  unsigned l = 0;
  int m = 0;
  while (str && str[m] && l < High(msg))
